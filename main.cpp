...
BOOL GameLoad(VOID)
{
	...
	//音楽を読み込む
	if (!LoadAudio(&TitleBGM, ".\\AUDIO\\opening1.mp3", 255, DX_PLAYTYPE_LOOP)) { return FALSE; }
	if (!LoadAudio(&PlayBGM, ".\\AUDIO\\Play.mp3", 255, DX_PLAYTYPE_LOOP)) { return FALSE; }
	if (!LoadAudio(&EndBGM, ".\\AUDIO\\silent_end.mp3", 255, DX_PLAYTYPE_LOOP)) { return FALSE; }

	if (!LoadAudio(&PlayerSE, ".\\AUDIO\\select01.mp3", 255, DX_PLAYTYPE_BACK)) { return FALSE; }
